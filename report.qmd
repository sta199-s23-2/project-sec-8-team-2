...
4. Next, we compute mean, median, and standard deviation of net worth for each potential outcome for our variables of interest.
```{r}
#| label: summary_stats
...
5. Using the above data, we create box plots that visualize how each variable affects the distribution of billionaire net worths. 
```{r}
...
6. We also created a box plot to visualize the difference in net worth between billionaires who did and did not inherit money. 
```{r}
...
7. Additionally, for each of our variables of interest, we compute the proportion of billionaires associated with each outcome. 
```{r}
...
8. Using the above data, we create pie chart visualizations. 
```{r}
...
wealth.category_prop |>
  ggplot(
    aes(x = "", y = cat_prop_vec, fill = cat_labs)
  ) +
...
wealth.industry_prop |>
  ggplot(
    aes(x = "", y = ind_prop_vec, fill = ind_labs)
  ) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  labs(
...
wealth.inherited_prop |>
  ggplot(
    aes(x = "", y = inh_prop_vec, fill = inh_labs)
  ) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  scale_fill_brewer(palette = "Pastel1") +
  labs(
    title = "Wealth Inheritance Proportions",
    x = "",
    y = "",
    fill = "Inheritance"
  ) +
...
Note that we exclude the citizenship pie chart because each citizenship type only accounts for a small proportion of all billionaires and the associated pie chart is not only difficult to read but not particularly meaningful. 
...
# Results
*Note that equations are not included because they are long, and given the abundance of terms in each equation, we felt that equations were not particularly useful for supporting the overall narrative. 
