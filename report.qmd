...
Next, we wrangle the data. 1. We coerce the variables to have valid names for our analysis.
...
wealth.category_prop |>
  ...
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.30, hjust = 0.25))
  
```

```{r}
#l label: bar-wealth-ind

wealth.industry_prop |>
  mutate(wealth.industry = c("n/a", "Construction", "Consumer", "Diversified Financial", 
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.30, hjust = 1))

```

```{r}
#l label: bar-inherited

wealth.inherited_prop |>
    mutate(wealth.industry = c("3rd gen", "4th gen", "5th gen", "father", "not inherited",
              "spouse/widow")) |>
  ggplot(
    aes(x = wealth.industry, y = prop_how)
  ) +
